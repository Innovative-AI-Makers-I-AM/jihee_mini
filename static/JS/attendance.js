...
// 버튼 이벤트 리스너 설정
entryButton.addEventListener('click', () => handleAction('attendance'));
outgoingButton.addEventListener('click', () => handleAction('leave'));
returningButton.addEventListener('click', () => handleAction('return'));
exitButton.addEventListener('click', () => handleAction('exit'));
cancelButton.addEventListener('click', () => {
    confirmationModal.style.display = 'none';
    setIsFaceDetected(false);
});
...
function renderEntry(entry) {
    if (entry.entry_time) {
        appendEntryEvent(entry, 'attendance');
    }
    if (entry.leave_time) {
        appendEntryEvent(entry, 'leave');
    }
    if (entry.return_time) {
        appendEntryEvent(entry, 'return');
    }
    if (entry.exit_time) {
        appendEntryEvent(entry, 'exit');
    }
}

// 서버에 요청을 보내고 화면에 항목을 추가
function postAttendanceEvent(type, user_id) {
    let url = `/${type}`;
    fetch(url, {
        method: 'POST',
        headers: {
    ...
    .then(updatedEntry => {
        ...
        fetchAndRenderEntries(); // 전체 목록을 다시 불러와서 갱신
        confirmationModal.style.display = 'none';
        setIsFaceDetected(false);
    })
    .catch(error => console.error(`Error updating ${type} time:`, error));
}

// 새로운 이벤트 항목 추가
function appendEntryEvent(entry, type) {
    let li = document.createElement('li');
    let eventTypeText = getEventTypeText(type, entry);
    li.textContent = `${entry.name} - ${eventTypeText}`;
    ...
    if (type === 'exit') {
        ...
        let totalLeaveTime = calculateTotalLeaveTime(entry);
        let li = document.createElement('li');
        li.textContent += `(근무시간) ${totalWorkTime} // (외출시간) ${totalLeaveTime}`;
        entriesList.appendChild(li);
    }
...
// 이벤트 타입에 따른 텍스트 생성
function getEventTypeText(type, entry) {
    switch(type) {
        case 'attendance':
            return `(입실) ${entry.entry_time}`;
        case 'exit':
            return `(퇴실) ${entry.exit_time}`;
        case 'leave':
            return `(외출) ${entry.leave_time}`;
        case 'return':
            return `(복귀) ${entry.return_time}`;
        default:
            return '';
    }
}
...
export function handleAction(type) {
    ...
    .then(data => {
        ...
        console.log('User ID:', userId);
        postAttendanceEvent(type, userId);
    })
    .catch(error => console.error('Error:', error));
}
